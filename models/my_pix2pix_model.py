...
class MyPix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        net = MyResnetGenerator(opt.input_nc, opt.output_nc, opt.ngf, 
                                norm_layer=networks.get_norm_layer(norm_type=opt.norm), 
                                use_dropout=(not opt.no_dropout), n_blocks=9)
        self.netG = networks.init_net(net, opt.init_type, opt.init_gain, self.gpu_ids)
...
class MyResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        model += [norm_layer(32),
                  nn.ReLU(True),
                  nn.Conv2d(32, 32, kernel_size=3, stride=2, padding=1, bias=use_bias), 
                  norm_layer(32),
                  nn.ReLU(True),
                  nn.Conv2d(32, 32, kernel_size=3, stride=2, padding=1, bias=use_bias),
                  norm_layer(32),
                  nn.ReLU(True),
                  nn.Conv2d(32, 64*64, kernel_size=3, stride=2, padding=1, bias=use_bias)]
        ...
        # self.output = nn.ModuleList([nn.Flatten() for i in range(268)])
